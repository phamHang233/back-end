...
from app.constants.network_constants import DEFAULT_CREDIT_SCORE, Chain
...
class MongoDB:
    def __init__(self, connection_url=None, database=MongoDBConfig.DATABASE):
        ...
        self._profiles_col = self.mongo_db[MongoDBCollections.profiles]
        self._users_col = self.mongo_db[MongoDBCollections.users]
        self._notifications_col = self.mongo_db[MongoDBCollections.notifications]

        self._multichain_wallets_credit_scores_col = self.mongo_db[MongoDBCollections.multichain_wallets_credit_scores]

        self._liquidates_col = self.mongo_db[MongoDBCollections.liquidates]
        ...
        self._followers_col = self.mongo_db[MongoDBCollections.followers]
    ...
    def get_tokens_by_id(self, token_id, chains=None, projection=None):
        filter_statement = {}
        chain_id = token_id.split('_')[0]
        all_chains = Chain.get_all_chain_id()
        if ((chains is None) and (chain_id in all_chains)) or ((chains is not None) and (chain_id in chains)):
            filter_statement.update({'_id': token_id})
        else:
            filter_statement.update({'idCoingecko': token_id})
            if chains is not None:
                filter_statement.update({'chainId': {'$in': chains}})
    ...
    @sync_log_time_exe(tag=TimeExeTag.database)
    def get_tokens_by_ids(self, token_ids, price_on_exchanges=None, projection=None):
        filter_statement = {'idCoingecko': {'$in': token_ids}}
        if price_on_exchanges:
            filter_statement['priceOnExchanges'] = {"$exists": True}
        projection_statement = self.get_projection_statement(projection)
        cursor = self._smart_contracts_col.find(filter_statement, projection=projection_statement)
        return cursor
    ...
    @sync_log_time_exe(tag=TimeExeTag.database)
    def get_tokens_by_keys(self, token_keys, price_on_exchanges=None, projection=None):
    ...
    def get_tokens_by_symbols(self, symbols, projection=None):
        # regex_symbols = [re.compile(symbol, re.IGNORECASE) for symbol in symbols]
        # filter_statement = {
        #     'idCoingecko': {'$exists': True},
        #     'symbol': {'$in': regex_symbols}
        # }
        filter_statement = {
            'idCoingecko': {'$exists': True},
            'symbol': {'$in': symbols}
    ...
    @sync_log_time_exe(tag=TimeExeTag.database)
    def get_tokens_by_symbols_with_price_on_exchange(self, symbols, projection=None):
        regex_symbols = [re.compile(symbol, re.IGNORECASE) for symbol in symbols]
        filter_statement = {
            'priceOnExchanges': {'$exists': True},
            'symbol': {'$in': regex_symbols}
        }
    ...
    def insert_new_wallets(self, new_wallets):
        ...
        for key in new_wallets:
            ...
            config_operators.append(UpdateOne(
                ...
                {'$set': {'_id': f'wallet_flags_{chain_id}', f'newTarget.{address}': True}},
                upsert=True
            ))
    ...
    @sync_log_time_exe(tag=TimeExeTag.database)
    def get_score_detail_change_logs(self, address: str):
        filter_statement = {'_id': address}
        projection_keys = {
            'creditScore': 'creditScoreChangeLogs',
            'assetsScore': 'creditScorex1ChangeLogs',
            'dappInteractionsScore': 'creditScorex2ChangeLogs',
            'transactionsScore': 'creditScorex3ChangeLogs',
            'liquidationScore': 'creditScorex4ChangeLogs',
            'loanScore': 'creditScorex5ChangeLogs',
            'circulatingAssetsScore': 'creditScorex6ChangeLogs',
            'trustworthinessAssetsScore': 'creditScorex7ChangeLogs'
        }
        projection_statement = self.get_projection_statement(list(projection_keys.values()))
        doc = self._multichain_wallets_credit_scores_col.find_one(filter_statement, projection=projection_statement)
        if not doc:
            return {key: {} for key in projection_keys.keys()}

        data = {}
        for idx, projection_map in enumerate(projection_keys.items()):
            key, score_change_log_field = projection_map
            score_logs = sort_log(doc.get(score_change_log_field))
            if idx != 0:
                for t, params in score_logs.items():
                    score_logs[t] = CreditScoreService.combine_score_element(element_idx=idx, params=params)
            data[key] = score_logs
    ...
    def get_score_details(self, address):
        filter_statement = {'_id': address}
        projection_keys = {
            'assets': 'creditScorex1',
            'dappInteractions': 'creditScorex2',
            'transactions': 'creditScorex3',
            'liquidation': 'creditScorex4',
            'loan': 'creditScorex5',
            'circulatingAssets': 'creditScorex6',
            'trustworthinessAssets': 'creditScorex7'
        }
        projection_statement = self.get_projection_statement(list(projection_keys.values()))
        doc = self._multichain_wallets_credit_scores_col.find_one(filter_statement, projection=projection_statement)
        ...
        for idx, projection_map in enumerate(projection_keys.items()):
            key, score_change_log_field = projection_map
            params = doc.get(score_change_log_field)
            data[key] = CreditScoreService.combine_score_element(element_idx=idx + 1, params=params)
    ...
    def get_config(self, key, projection=None):
        ...
        config = self._configs_col.find_one({'_id': key}, projection=projection)
        return config
