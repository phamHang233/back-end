...
from app.artifacts.abis.erc20_abi import ERC20_ABI
from app.artifacts.abis.lp_abi import LP_TOKEN_ABI
from app.artifacts.abis.multicall_v3_abi import MULTICALL_V3_ABI
from app.constants.network_constants import Chain, ProviderURI
from app.utils.list_dict_utils import all_equal
from app.utils.logger_utils import get_logger
from job.services.batch_queries_service import add_rpc_call, decode_data_response, decode_data_response_ignore_error
...
class MulticallContract:
    ...
    on_chains_v3 = {
        Chain.ETH: '0xca11bde05977b3631167028862be2a173976ca11',
        ...
        Chain.FTM: '0xca11bde05977b3631167028862be2a173976ca11',
        ...
        '0x5aff': '0xca11bde05977b3631167028862be2a173976ca11'
    }
...
def _test_multicall(_w3: Web3, multicall_contract, wallets, tokens):
    ...
    for wallet in wallets:
        ...
        w3_multicall = W3Multicall(_w3, address=MulticallContract.get_multicall_contract(Chain.ARBITRUM))
        for token in tokens:
...
def query_multi_contracts(n_times=20, wallets_batch_size=5, wallets_distribute_similar=True):
    ...
    start_time = time.time()
    client_querier = ClientQuerier(provider_url=ProviderURI.mapping[Chain.ARBITRUM])
    ...
    start_time = time.time()
    _w3 = Web3(Web3.HTTPProvider(ProviderURI.mapping[Chain.ARBITRUM]))
    starting_time_2_1 = time.time() - start_time
    multicall_address = MulticallContract.get_multicall_contract(Chain.ARBITRUM)
