...
def get_last_monday_from_epoch(epoch_time):

    dt_object = datetime.fromtimestamp(epoch_time - 7 * TimeConstants.A_HOUR)

    days_since_monday = dt_object.weekday()
    last_monday = dt_object - timedelta(days=days_since_monday)

    last_monday_start = last_monday.replace(hour=0, minute=0, second=0, microsecond=0)

    last_monday_epoch = int(last_monday_start.timestamp())

    return last_monday_epoch + 7 * TimeConstants.A_HOUR
